...
// Risk Score Formula:
// Risk Score = (0.30 x Login Decay) 
//            + (0.25 x Submission Latency) 
//            + (0.25 x Engagement Drop) 
//            + (0.20 x Grade Decline)


export const calculateRisk = (student: Student): { riskScore: number; riskLevel: RiskLevel; gradeTrend: 'Improving' | 'Stable' | 'Declining'; engagementScore: number; redFlags: string[] } => {
    ...
    if (loginDrop > 0.5) redFlags.push('Significant drop in login frequency');
    ...
    if (latencyScoreRaw > 0.3) redFlags.push('Recent missed or late assignments');
    ...
    if (engDrop > 0.5) redFlags.push('Drastic decline in engagement');
    ...
    if (gradeDrop > 0.1) redFlags.push('Declining academic performance');

    // Calculate Grade Trend
    let gradeTrend: 'Improving' | 'Stable' | 'Declining' = 'Stable';
    if (avgGradeNew > avgGradeOld + 2) gradeTrend = 'Improving';
    else if (avgGradeNew < avgGradeOld - 2) gradeTrend = 'Declining';

    // Calculate Engagement Score (0-100)
    // Max expected daily: 120min video + 30pts (3 posts) = 150
    const engagementScore = Math.min(100, Math.round((avgEngNew / 150) * 100));
